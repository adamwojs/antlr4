	...
	public static abstract class MultipleAlternativesWithCommonLabel extends BaseParserTestDescriptor {
		...
		/**
		 ...
		 e returns [int v]
		   ...
		   | INT         {$v = $INT.int;}                   # anInt
		   | '(' e ')'   {$v = $e.v;}                       # parens
		   | left=e INC  {<Cast("UnaryContext","$ctx"):CallMethod("INC"):Concat(" != null"):Assert()>$v = $left.v + 1;}      # unary
		   | left=e DEC  {<Cast("UnaryContext","$ctx"):CallMethod("DEC"):Concat(" != null"):Assert()>$v = $left.v - 1;}      # unary
	...
	public static abstract class PrefixOpWithActionAndLabel extends BaseParserTestDescriptor {
		...
		/**
		 ...
		 e returns [<StringType()> result]
		     :   ID '=' e1=e    {$result = <AppendList(["\"(\"", "$ID.text", "\"=\"", "$e1.result", "\")\""])>;}
		     |   ID             {$result = $ID.text;}
		     |   e1=e '+' e2=e  {$result = <AppendList(["\"(\"", "$e1.result", "\"+\"", "$e2.result", "\")\""])>;}
		     ;
		 ID : 'a'..'z'+ ;
		 INT : '0'..'9'+ ;
		 WS : (' '|'\n') -> skip ;
