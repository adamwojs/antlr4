...
use ANTLR\v4\Runtime\ATN\ATNState;
use ANTLR\v4\Runtime\Exception\FailedPredicateException;
...
use ANTLR\v4\Runtime\Misc\IntSet;
...
class DefaultErrorStrategy implements ANTLRErrorStrategy
{
     ...
     */
    protected $lastErrorStates = null;
     ...
     */
    protected $nextTokensContext = null;
    ...
    public function recover(Parser $recognizer, RecognitionException $e): void
    {
        ...
        $this->consumeUntil($recognizer, $this->getErrorRecoverySet($recognizer));
    }
    ...
    public function sync(Parser $recognizer): void
    {
        ...
        $la = $recognizer->getInputStream()->LA(1);

        $nextTokens = $recognizer->getATN()->nextTokensForATNState($s);
        ...
        switch ($s->getStateType()) {
            ...
            case ATNState::STAR_LOOP_BACK:
                ...
                $whatFollowsLoopIterationOrRule = $recognizer->getExpectedTokens()->or(
    ...
    {
        /** @var \ANTLR\v4\Runtime\TokenStream $tokens */
        ...
        if ($tokens !== null) {
            if ($e->getStartToken()->getType() === Token::EOF) {
                ...
                $input = $tokens->getTextBetweenTokens($e->getStartToken(), $e->getOffendingToken());
            }
        }
     ...
     *
     * @param \ANTLR\v4\Runtime\Parser $recognizer the parser instance
     * @param \ANTLR\v4\Runtime\Exception\InputMismatchException $e the recognition exception
     */
    protected function reportInputMismatch(Parser $recognizer, InputMismatchException $e): void
    {
        $msg = sprintf(
            ...
            $e->getExpectedTokens()->toStringUsingVocabulary($recognizer->getVocabulary())
        );
     ...
     * @see #reportError
     *
     * @param \ANTLR\v4\Runtime\Recognizer $recognizer the parser instance
     * @param \ANTLR\v4\Runtime\Exception\FailedPredicateException $e the recognition exception
     */
    protected function reportFailedPredicate(Parser $recognizer, FailedPredicateException $e): void
    {
        $ruleName = $recognizer->getRuleNames()[$recognizer->getRuleContext()->getRuleIndex()];

        $recognizer->notifyErrorListeners("rule $ruleName {$e->getMessage()}", $e->getOffendingToken(), $e);
    }
    ...
    protected function reportUnwantedToken(Parser $recognizer): void
    {
        ...
        $tokenName = $this->getTokenErrorDisplay($currentToken);
        $expecting = $this->getExpectedTokens($recognizer);

        $msg = 'extraneous input ' . $tokenName . ' expecting '
            . $expecting->toStringUsingVocabulary($recognizer->getVocabulary());
    ...
    protected function reportMissingToken(Parser $recognizer): void
    {
        ...
        $msg = 'missing ' . $expecting->toStringUsingVocabulary($recognizer->getVocabulary())
            . ' at ' . $this->getTokenErrorDisplay($currentToken);
     ...
     * @param \ANTLR\v4\Runtime\Parser $recognizer the parser instance
     *
     * @return bool {@code true} if single-token insertion is a viable recovery
    ...
    protected function singleTokenInsertion(Parser $recognizer): bool
    {
        ...
        $nextState = $atn->states[$recognizer->getState()]->transition(0)->target;
        $expectingAtLL2 = $atn->nextTokensForATNStateAndRuleContext(
            $nextState, $recognizer->getContext()
        );
     ...
     * @param \ANTLR\v4\Runtime\Parser $recognizer the parser instance
     *
     * @return \ANTLR\v4\Runtime\Token|null the successfully matched {@link Token} instance if single-token
    ...
    private function getExpectedTokens(Parser $recognizer): IntervalSet
    {
     ...
     *
     * @param \ANTLR\v4\Runtime\Token|null $t
     *
     * @return string
     */
    ...
    protected function getErrorRecoverySet(Parser $recognizer): IntervalSet
    {
        ...
        while ($ctx !== null && $ctx->invokingState >= 0) {
            ...
            $follow = $atn->nextTokensForATNState($rt->followState);
     ...
     *
     * @param \ANTLR\v4\Runtime\Parser $recognizer
     * @param \ANTLR\v4\Runtime\Misc\IntSet $set
     */
    protected function consumeUntil(Parser $recognizer, IntSet $set): void
    {
