...
final class PredictionMode
{
     ...
     * not SLL.
     *
     * <p>When using this prediction mode, the parser will either return a correct
     ...
     *
     * <p>This prediction mode does not provide any guarantees for prediction
     ...
     * inputs.
     *
     * <p>When using this prediction mode, the parser will make correct decisions
     ...
     *
     * <p>This prediction mode may be used for diagnosing ambiguities during
    ...
    public static function hasSLLConflictTerminatingPrediction(int $mode, ATNConfigSet $configs): bool
    {
        ...
        if ($mode === self::SLL) {
            ...
            if ($configs->hasSemanticContext) {
                // duplicated configs, tossing out semantic predicates
                $duplicatedConfig = new ATNConfigSet();
                foreach ($configs as $c) {
                    $duplicatedConfig->add(ATNConfig::copyWithoutSemanticContext($c, SemanticContext::NONE()));
                }

                $configs = $duplicatedConfig;
            }
     ...
     * context).
     *
     * @param \ANTLR\v4\Runtime\ATN\ATNConfigSet $configs the configuration set to test
     *
     * @return bool {@code true} if any configuration in {@code configs} is in a
     ...
     * @param \ANTLR\v4\Runtime\ATN\ATNConfigSet $configs the configuration set to test
     *
     * @return bool {@code true} if all configurations in {@code configs} are in a
     * {@link RuleStopState}, otherwise {@code false}
     */
     ...
     *
     * @param \ANTLR\v4\Runtime\Misc\BitSet[] $altsets a collection of alternative subsets
     *
     * @return bool {@code true} if every {@link BitSet} in {@code altsets} has
     ...
     * @param \ANTLR\v4\Runtime\Misc\BitSet[] $altsets a collection of alternative subsets
     *
     * @return bool {@code true} if every member of {@code altsets} is equal to the
     * others, otherwise {@code false}
     */
    public static function allSubsetsEqual(array $altsets): bool
    {
        $it = new ArrayIterator($altsets);
     ...
     * @return int
     */
    public static function getUniqueAlt($altsets): int
    {
     ...
     * @param \ANTLR\v4\Runtime\Misc\BitSet[] $altsets a collection of alternative subsets\
     *
     * @return \ANTLR\v4\Runtime\Misc\BitSet the set of represented alternatives in {@code altsets}
     */
    public static function getAlts($altsets): BitSet
    {
        $all = new BitSet();
        foreach ($altsets as $alts) {
            $all->or($alts);
     ...
     */
    public static function getAltsFromConfigs(ATNConfigSet $configs): BitSet
    {
        $all = new BitSet();
        foreach ($configs as $config) {
            $all->set($config->alt);
    ...
    {
        $configToAlts = new AltAndContextMap();
        foreach ($configs as $config) {
            $alts = $configToAlts->get($config);

            if ($alts === null) {
                $alts = new BitSet();
                $configToAlts->put($config, $alts);
            }
    ...
    public static function getStateToAltMap(ATNConfigSet $configs): Map
    {
        $map = new Map();
        foreach ($configs as $config) {
            $alts = $map->get($config->state, null);
            if ($alts === null) {
                $alts = new BitSet();
                $map->put($config->state, $alts);
            }
    ...
    {
        $map = self::getStateToAltMap($configs);
        foreach ($map->values() as $alts) {
            /** @var \ANTLR\v4\Runtime\Misc\BitSet $alts */
            if ($alts->cardinality() === 1) {
                return true;
            }
        }
